...
def _process_pdf(file_content: bytes, job_id: str, filename: str, temp_path: str) -> dict:
    ...
    try:
        ...
        doc.close()
    # ── Detect if scanned/handwritten ─────────────────────────────────
    total_chars  = sum(p.get("char_count", 0) for p in pages)
    avg_chars    = total_chars / max(len(pages), 1)

    # Remove watermark-only text from count
    all_text = " ".join(p.get("text", "") for p in pages).lower()
    watermarks = ["scanned by camscanner", "camscanner", "adobe scan", "microsoft lens"]
    cleaned = all_text
    for w in watermarks:
        cleaned = cleaned.replace(w, "")
    meaningful_chars = len([c for c in cleaned if c.isalpha()])
    meaningful_avg   = meaningful_chars / max(len(pages), 1)

    is_scanned = avg_chars < 50 or meaningful_avg < 30
    return {
        "job_id": job_id,
        "status": "done",
        "filename": filename,
